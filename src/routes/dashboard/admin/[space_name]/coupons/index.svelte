<script lang="ts">
  ...
  async function handleCreateCoupon() {
    ...
    if (!sellerShortname) {
      errorToastMessage(
        t("admin.seller_required", "Please select a seller or global"),
      );
      return;
    }
      ...
      <div class="modal-body">
        <div class="form-grid">
          <div class="form-group">
            <label for="seller">{t("admin.seller", "Seller")} *</label>
            <select id="seller" bind:value={sellerShortname}>
              <option value="" disabled
                >{t("admin.select_seller", "Select a seller")}</option
              >
              <option value="global"
                >{t("admin.global_coupon", "Global Coupon")}</option
              >
              {#each getUniqueSellers() as seller}
